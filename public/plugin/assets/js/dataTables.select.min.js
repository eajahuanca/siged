...
! function(a) { "function" == typeof define && define.amd ? define(["jquery", "datatables.net"], function(b) { return a(b, window, document) }) : "object" == typeof exports ? module.exports = function(b, c) { return b || (b = window), c && c.fn.dataTable || (c = require("datatables.net")(b, c).$), a(c, b, b.document) } : a(jQuery, window, document) }(function(a, b, c, d) {
    ...
    function e(a, b, c) {
        var d, e, f, g = function(b, c) {
                ...
                var e = !1;
                return a.columns(":visible").indexes().filter(function(a) { return a === b && (e = !0), a === c ? (e = !1, !0) : e })
            },
            h = function(b, c) {
                ...
                var f = !1;
                return d.filter(function(a) { return a === b && (f = !0), a === c ? (f = !1, !0) : f })
            };
    ...
    function g(c) {
        ...
        d.on("mousedown.dtSelect", f, function(a) { a.shiftKey && d.css("-moz-user-select", "none").one("selectstart.dtSelect", f, function() { return !1 }) }).on("mouseup.dtSelect", f, function(a) { d.css("-moz-user-select", "") }).on("click.dtSelect", f, function(d) {
    ...
    function h(b, c, d, e) { e && !b.flatten().length || (d.unshift(b), a(b.table().node()).triggerHandler(c + ".dt", d)) }

    function i(b) {
        var c = b.settings()[0];
        if (c._select.info && c.aanFeatures.i) {
            var d = a('<span class="select-info"/>'),
                e = function(c, e) { d.append(a('<span class="select-item"/>').append(b.i18n("select." + c + "s", { _: "%d " + c + "s selected", 0: "", 1: "1 " + c + " selected" }, e))) };
    ...
    function j(b) {
        var c = new o.Api(b);
        b.aoRowCreatedCallback.push({ fn: function(c, d, e) { var f, g, h = b.aoData[e]; for (h._select_selected && a(c).addClass(b._select.className), f = 0, g = b.aoColumns.length; g > f; f++)(b.aoColumns[f]._select_selected || h._selected_cells && h._selected_cells[f]) && a(h.anCells[f]).addClass(b._select.className) }, sName: "select-deferRender" }), c.on("preXhr.dt.dtSelect", function() {
            var a = c.rows({ selected: !0 }).ids(!0).filter(function(a) { return a !== d }),
                b = c.cells({ selected: !0 }).eq(0).map(function(a) { var b = c.row(a.row).id(!0); return b ? { row: b, column: a.column } : d }).filter(function(a) { return a !== d });
            c.one("draw.dt.dtSelect", function() { c.rows(a).select(), b.any() && b.each(function(a) { c.cells(a.row, a.column).select() }) })
        }), c.on("draw.dtSelect.dt select.dtSelect.dt deselect.dtSelect.dt", function() { i(c) }), c.on("destroy.dtSelect", function() { f(c), c.off(".dtSelect") })
    }
    ...
    function n(a, b) { return function(c) { return c.i18n("buttons." + a, b) } }
    ...
    o.select = {}, o.select.version = "1.1.2", o.select.init = function(b) {
        var c = b.settings()[0],
            ...
            l = "td, th",
            m = "selected";
        c._select = {}, g === !0 ? i = "os" : "string" == typeof g ? i = g : a.isPlainObject(g) && (g.blurable !== d && (j = g.blurable), g.info !== d && (k = g.info), g.items !== d && (h = g.items), g.style !== d && (i = g.style), g.selector !== d && (l = g.selector), g.className !== d && (m = g.className)), b.select.selector(l), b.select.items(h), b.select.style(i), b.select.blurable(j), b.select.info(k), c._select.className = m, a.fn.dataTable.ext.order["select-checkbox"] = function(b, c) { return this.api().column(c, { order: "index" }).nodes().map(function(c) { return "row" === b._select.items ? a(c).parent().hasClass(b._select.className) : "cell" === b._select.items ? a(c).hasClass(b._select.className) : !1 }) }, a(b.table().node()).hasClass("selectable") && b.select.style("os")
    ...
    return p("select()", function() { return this.iterator("table", function(a) { o.select.init(new o.Api(a)) }) }), p("select.blurable()", function(a) { return a === d ? this.context[0]._select.blurable : this.iterator("table", function(b) { b._select.blurable = a }) }), p("select.info()", function(a) { return i === d ? this.context[0]._select.info : this.iterator("table", function(b) { b._select.info = a }) }), p("select.items()", function(a) { return a === d ? this.context[0]._select.items : this.iterator("table", function(b) { b._select.items = a, h(new o.Api(b), "selectItems", [a]) }) }), p("select.style()", function(a) {
        ...
        }), this.iterator("table", function(a, b) { h(c, "select", ["column", c[b]], !0) }), this)
        ...
        }), this.iterator("table", function(a, b) { h(c, "select", ["cell", c[b]], !0) }), this)
    }), q("rows().deselect()", "row().deselect()", function() { var b = this; return this.iterator("row", function(b, c) { b.aoData[c]._select_selected = !1, a(b.aoData[c].nTr).removeClass(b._select.className) }), this.iterator("table", function(a, c) { h(b, "deselect", ["row", b[c]], !0) }), this }), q("columns().deselect()", "column().deselect()", function() {
        var b = this;
        ...
        }), this.iterator("table", function(a, c) { h(b, "deselect", ["column", b[c]], !0) }), this
        ...
        }), this.iterator("table", function(a, c) { h(b, "deselect", ["cell", b[c]], !0) }), this
        ...
        selectNone: {
            ...
            action: function() { l(this.settings()[0], !0) },
            init: function(a, b, c) {
                var d = this;
                a.on("draw.dt.DT select.dt.DT deselect.dt.DT", function() {
                    var b = a.rows({ selected: !0 }).flatten().length + a.columns({ selected: !0 }).flatten().length + a.cells({ selected: !0 }).flatten().length;
        ...
        o.ext.buttons["select" + b + "s"] = {
            ...
            init: function(a, b, d) {
                var e = this;
                a.on("selectItems.dt.DT", function(a, b, d) { e.active(d === c) })
            }
        }
    }), a(c).on("preInit.dt.dtSelect", function(a, b) { "dt" === a.namespace && o.select.init(new o.Api(b)) }), o.select
});